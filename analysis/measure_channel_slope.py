# a function to measure chanbel slope for intervals in tulane data
# Eric Barefoot
# Nov 2020
...
# mtch = np.isin(agu_data['channelMasksqv10'].attrs['IDs'], agu_data['topoqv10'].attrs['IDs'] - 4)
# topo10[~msk10[mtch]] = np.nan
#
# for istr, jstr, idat, jdat in zip(topstr, mskstr, topdat, mskdat):
#     mtch = np.isin(agu_data[jstr].attrs['IDs'], agu_data[istr].attrs['IDs'] - 4)
#     idat[~jdat[mtch]] = np.nan
...
for i in tqdm(range(topo10.shape[0])):
    slps10[i], angs10[i] = ttools.measure_slopes(topo10[i], apex10, c110, c210, ntheta, nanthresh = 0.2)
...
for i in tqdm(range(topo15.shape[0])):
    slps15[i], angs15[i] = ttools.measure_slopes(topo15[i], apex15, c115, c215, ntheta, nanthresh = 0.2)
...
for i in tqdm(range(topo30.shape[0])):
    slps30[i], angs30[i] = ttools.measure_slopes(topo30[i], apex15, c115, c215, ntheta, nanthresh = 0.2)
...
slpdat.to_csv(os.path.join(dir_path, 'data/derived_data/ChannelSlopes.csv'), index = False)
